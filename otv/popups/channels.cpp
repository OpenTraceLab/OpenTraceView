	...
	check_box_mapper_(this)
{
	...
	const shared_ptr<opentrace::Device> device = session_.device()->device();
	assert(device);
...
{
	otv::widgets::Popup::showEvent(event);
