...
DecoderMenu::DecoderMenu(QWidget *parent, const char* input, bool first_level_decoder) :
	...
	mapper_(this)
{
	GSList *li = g_slist_sort(g_slist_copy((GSList*)otd_decoder_list()), decoder_name_cmp);

	for (GSList *l = li; l; l = l->next) {
		const otd_decoder *const d = (otd_decoder*)l->data;
		assert(d);
...
{
	return strcmp(((const otd_decoder*)a)->name, ((const otd_decoder*)b)->name);
}
...
void DecoderMenu::on_action(QObject *action)
{
	...
	otd_decoder *const dec = (otd_decoder*)((QAction*)action)->data().value<void*>();
	assert(dec);
