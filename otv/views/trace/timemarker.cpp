...
#include "otv/widgets/timestampspinbox.hpp"
#include "ruler.hpp"
#include "view.hpp"
...
TimeMarker::TimeMarker(
	View &view, const QColor &color, const otv::util::Timestamp& time) :
	TimeItem(view),
	color_(color),
	time_(time),
...
const otv::util::Timestamp TimeMarker::time() const
{
	return time_;
}

void TimeMarker::set_time(const otv::util::Timestamp& time)
{
	time_ = time;
...
float TimeMarker::get_x() const
{
	...
	return roundf((float)((time_ - view_.offset()) / view_.scale())) + 0.5f;
}
...
otv::widgets::Popup* TimeMarker::create_popup(QWidget *parent)
{
	...
	value_widget_ = new otv::widgets::TimestampSpinBox(parent);
	value_widget_->setValue(view_.ruler()->get_ruler_time_from_absolute_time(time_));

	connect(value_widget_, SIGNAL(valueChanged(const otv::util::Timestamp&)),
		this, SLOT(on_value_changed(const otv::util::Timestamp&)));
...
void TimeMarker::on_value_changed(const otv::util::Timestamp& value)
{
