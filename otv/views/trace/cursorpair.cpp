...
void CursorPair::set_time(const otv::util::Timestamp& time)
{
	const otv::util::Timestamp delta = second_->time() - first_->time();
	first_->set_time(time);
	second_->set_time(time + delta);
}

const otv::util::Timestamp CursorPair::time() const
{
	return 0;
}
...
const otv::util::Timestamp CursorPair::delta(const otv::util::Timestamp& other) const
{
...
otv::widgets::Popup* CursorPair::create_popup(QWidget *parent)
{
	(void)parent;
	return nullptr;
}
...
QString CursorPair::format_string_sub(int time_precision, int freq_precision, bool show_unit)
{
	...
	const otv::util::SIPrefix prefix = view_.tick_prefix();
	const otv::util::Timestamp diff = abs(second_->time() - first_->time());

	const QString time = Ruler::format_time_with_distance(
		diff, diff, prefix, (show_unit ? view_.time_unit() : otv::util::TimeUnit::None),
		time_precision, false);

	// We can only show a frequency when there's a time base
	if (view_.time_unit() == otv::util::TimeUnit::Time) {
		int items = 0;

		if (show_frequency_) {
			const QString freq = util::format_value_si(
				1 / diff, otv::util::SIPrefix::unspecified,
		...
		if (show_samples_) {
			const QString samples = QString::number(
				(uint64_t)(diff * view_.session().get_samplerate()));
			if (items > 0)
