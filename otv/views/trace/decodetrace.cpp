...
#include <otv/widgets/decodergroupbox.hpp>
#include <otv/widgets/decodermenu.hpp>
#include <otv/widgets/flowlayout.hpp>
...
using otv::data::decode::Annotation;
using otv::data::decode::AnnotationClass;
using otv::data::decode::Row;
using otv::data::decode::DecodeChannel;
using otv::data::DecodeSignal;
...
DecodeTrace::DecodeTrace(otv::Session &session,
...
void DecodeTrace::update_stack_button()
{
	...
	if (!stack.empty()) {
		const otd_decoder* d = stack.back()->get_otd_decoder();

		if (d->outputs) {
			otv::widgets::DecoderMenu *const decoder_menu =
				new otv::widgets::DecoderMenu(stack_button_, (const char*)(d->outputs->data));
			connect(decoder_menu, SIGNAL(decoder_selected(otd_decoder*)),
				this, SLOT(on_stack_decoder(otd_decoder*)));
...
pair<double, double> DecodeTrace::get_pixels_offset_samples_per_pixel() const
{
	...
	const double pixels_offset =
		((view->offset() - decode_signal_->start_time()) / scale);
	...
	QWidget *parent, QFormLayout *form)
{
	...
	assert(dec);
	const otd_decoder *const decoder = dec->get_otd_decoder();
	assert(decoder);

	const bool decoder_deletable = index > 0;

	otv::widgets::DecoderGroupBox *const group =
		new otv::widgets::DecoderGroupBox(
	...
	const DecodeChannel *ch)
{
	...
	selector->addItem("0", QVariant::fromValue((int)OTD_INITIAL_PIN_LOW));
	selector->addItem("1", QVariant::fromValue((int)OTD_INITIAL_PIN_HIGH));
	selector->addItem("X", QVariant::fromValue((int)OTD_INITIAL_PIN_SAME_AS_SAMPLE0));
...
void DecodeTrace::on_stack_decoder(otd_decoder *decoder)
{
...
void DecodeTrace::on_export_row_with_cursor()
{
	...
	const otv::util::Timestamp& start_time = view->cursors()->first()->time();
	const otv::util::Timestamp& end_time = view->cursors()->second()->time();
