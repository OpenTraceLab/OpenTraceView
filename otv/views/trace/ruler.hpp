...
#ifndef PULSEVIEW_OTV_VIEWS_TRACE_RULER_HPP
#define PULSEVIEW_OTV_VIEWS_TRACE_RULER_HPP
...
public:
	 ...
	 *   - If 'unit' is 'TimeUnit::Samples', 'otv::util::format_time_si_adjusted()'
	 *     is used to format 't'.
	 *   - If a zoomed out view is detected (determined by 'precision' and
	 *     'distance'), 'otv::util::format_time_minutes() is used.
	 *   - For timestamps "near the origin" (determined by 'distance'),
	 *    'otv::util::format_time_si_adjusted()' is used.
	 *   - If none of the previous was true, 'otv::util::format_time_minutes()'
	 *     is used again.
	 *
	...
	static QString format_time_with_distance(
		...
		otv::util::SIPrefix prefix = otv::util::SIPrefix::unspecified,
		otv::util::TimeUnit unit = otv::util::TimeUnit::Time,
		unsigned precision = 0,
		bool sign = true);

	otv::util::Timestamp get_absolute_time_from_x_pos(uint32_t x) const;
	otv::util::Timestamp get_ruler_time_from_x_pos(uint32_t x) const;

	otv::util::Timestamp get_ruler_time_from_absolute_time(const otv::util::Timestamp& abs_time) const;
	otv::util::Timestamp get_absolute_time_from_ruler_time(const otv::util::Timestamp& ruler_time) const;
...
private:
	 ...
	 */
	std::optional<TickPositions> tick_position_cache_;
...
#endif // PULSEVIEW_OTV_VIEWS_TRACE_RULER_HPP
