...
#ifndef PULSEVIEW_OTV_VIEWS_TRACE_DECODETRACE_HPP
#define PULSEVIEW_OTV_VIEWS_TRACE_DECODETRACE_HPP
...
public:
	DecodeTrace(otv::Session &session, shared_ptr<SignalBase> signalbase,
		int index);
...
private Q_SLOTS:
	...
	void on_stack_decoder(otd_decoder *decoder);
...
private:
	...
	list< shared_ptr<otv::binding::Decoder> > bindings_;

	const Row* selected_row_;
	pair<uint64_t, uint64_t> selected_sample_range_;

	vector<otv::widgets::DecoderGroupBox*> decoder_forms_;
...
#endif // PULSEVIEW_OTV_VIEWS_TRACE_DECODETRACE_HPP
