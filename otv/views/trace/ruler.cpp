...
QString Ruler::format_time_with_distance(
	const otv::util::Timestamp& distance,
	const otv::util::Timestamp& t,
	otv::util::SIPrefix prefix,
	otv::util::TimeUnit unit,
	unsigned precision,
	bool sign)
{
	...
	if (unit == otv::util::TimeUnit::Samples)
		return otv::util::format_time_si_adjusted(t, prefix, precision, "sa", sign);

	QString unit_string;
	if (unit == otv::util::TimeUnit::Time)
		unit_string = "s";
	// Note: In case of otv::util::TimeUnit::None, unit_string remains empty

	// View zoomed way out -> low precision (0), big distance (>=60s)
	// -> DD:HH:MM
	if ((precision == 0) && (distance >= limit))
		return otv::util::format_time_minutes(t, 0, sign);
	...
	// -> HH:MM:SS.mmm... or xxxx (si unit) if less than limit seconds
	if (abs(t) < limit)
		return otv::util::format_time_si_adjusted(t, prefix, precision, unit_string, sign);
	else
		return otv::util::format_time_minutes(t, precision, sign);
}

otv::util::Timestamp Ruler::get_absolute_time_from_x_pos(uint32_t x) const
{
	return view_.offset() + ((double)x + 0.5) * view_.scale();
}

otv::util::Timestamp Ruler::get_ruler_time_from_x_pos(uint32_t x) const
{
	return view_.ruler_offset() + ((double)x + 0.5) * view_.scale();
}

otv::util::Timestamp Ruler::get_ruler_time_from_absolute_time(const otv::util::Timestamp& abs_time) const
{
	return abs_time + view_.zero_offset();
}

otv::util::Timestamp Ruler::get_absolute_time_from_ruler_time(const otv::util::Timestamp& ruler_time) const
{
...
void Ruler::paintEvent(QPaintEvent*)
{
	if (!tick_position_cache_) {
		auto ffunc = [this](const otv::util::Timestamp& t) {
...
TickPositions Ruler::calculate_tick_positions(
	const otv::util::Timestamp& major_period,
	const otv::util::Timestamp& offset,
	const double scale,
	const int width,
	const unsigned int minor_tick_count,
	function<QString(const otv::util::Timestamp&)> format_function)
{
	TickPositions tp;

	const otv::util::Timestamp minor_period = major_period / minor_tick_count;
	const otv::util::Timestamp first_major_division = floor(offset / major_period);
	const otv::util::Timestamp first_minor_division = ceil(offset / minor_period);
	const otv::util::Timestamp t0 = first_major_division * major_period;

	int division = (int)(round(first_minor_division -
		first_major_division * minor_tick_count)) - 1;

	double x;

	do {
		otv::util::Timestamp t = t0 + division * minor_period;
		x = ((t - offset) / scale);
...
{
	tick_position_cache_ = std::nullopt;
}
