...
#ifndef PULSEVIEW_OTV_VIEWS_TRACE_VIEW_HPP
#define PULSEVIEW_OTV_VIEWS_TRACE_VIEW_HPP
...
#include <otv/views/viewbase.hpp>
...
private:
	static const otv::util::Timestamp MaxScale;
	static const otv::util::Timestamp MinScale;
...
public:
	 ...
	 */
	const otv::util::Timestamp& offset() const;

	/**
	 * Returns the ruler version of the time offset of the left edge
	 * of the view in seconds.
	 */
	const otv::util::Timestamp& ruler_offset() const;

	void set_zero_position(const otv::util::Timestamp& position);

	void reset_zero_position();

	otv::util::Timestamp zero_offset() const;
	 ...
	 */
	otv::util::SIPrefix tick_prefix() const;
	 ...
	 */
	const otv::util::Timestamp& tick_period() const;
	 ...
	 */
	void set_scale_offset(double scale, const otv::util::Timestamp& offset);

	vector< shared_ptr<otv::data::SignalData> > get_visible_data() const;

	pair<otv::util::Timestamp, otv::util::Timestamp> get_time_extents() const;
	 ...
	 */
	void set_cursors(otv::util::Timestamp& first, otv::util::Timestamp& second);
	 ...
	 */
	shared_ptr<Flag> add_flag(const otv::util::Timestamp& time);
...
private:
	void get_scroll_layout(double &length, otv::util::Timestamp &offset) const;
...
private Q_SLOTS:
	 ...
	 * signal if needed.
	 */
	void set_offset(const otv::util::Timestamp& offset, bool force_update = false);
	 ...
	 * signal if needed.
	 */
	void set_tick_prefix(otv::util::SIPrefix tick_prefix);
	 ...
	 * signal if needed.
	 */
	void set_tick_period(const otv::util::Timestamp& tick_period);

	/**
	 * Sets the 'time_unit' member and emits the 'time_unit_changed'
	 * signal if needed.
	 */
	void set_time_unit(otv::util::TimeUnit time_unit);
...
private:
	...
	otv::util::Timestamp offset_;
	/// The ruler version of the time offset in seconds.
	otv::util::Timestamp ruler_offset_;
	/// The offset of the zero point in seconds.
	otv::util::Timestamp zero_offset_;
	...
	otv::util::Timestamp tick_period_;
	otv::util::SIPrefix tick_prefix_;
	...
	otv::util::Timestamp offset_at_acq_start_;
...
#endif // PULSEVIEW_OTV_VIEWS_TRACE_VIEW_HPP
