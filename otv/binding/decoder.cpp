...
Decoder::Decoder(
	shared_ptr<otv::data::DecodeSignal> decode_signal,
	...
	decoder_(decoder)
{
	...
	const otd_decoder *const dec = decoder_->get_otd_decoder();
	assert(dec);

	for (GSList *l = dec->options; l; l = l->next) {
		const otd_decoder_option *const opt = (otd_decoder_option*)l->data;
...
shared_ptr<Property> Decoder::bind_enum(
	...
	const otd_decoder_option *option,
...
Glib::VariantBase Decoder::getter(const char *id)
{
	...
	if (iter != options.end())
		...
		assert(decoder_->get_otd_decoder());

		// Get the default value if not
		for (GSList *l = decoder_->get_otd_decoder()->options; l; l = l->next) {
			const otd_decoder_option *const opt = (otd_decoder_option*)l->data;
