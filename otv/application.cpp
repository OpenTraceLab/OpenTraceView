...
{
	const struct otd_decoder *sda, *sdb;

	sda = (const struct otd_decoder *)a;
	sdb = (const struct otd_decoder *)b;
...
{
	setApplicationVersion(OTV_VERSION_STRING);
	setApplicationName("OpenTraceView");
...
{
	if (key == otv::GlobalSettings::Key_General_Language)
		switch_language(value.toString());
}

void Application::collect_version_info(otv::DeviceManager &device_manager)
{
	...
	version_info_.emplace_back("glibmm", OTV_GLIBMM_VERSION);
	...
	version_info_.emplace_back("libsigrok", QString("%1/%2 (rt: %3/%4)")
		.arg(otc_package_version_string_get(), otc_lib_version_string_get(),
		otc_package_version_string_get(), otc_lib_version_string_get()));

	GSList *l_orig = otc_buildinfo_libs_get();
	...
	char *host = otc_buildinfo_host_get();
	version_info_.emplace_back(" - Host", QString(host));
	g_free(host);

	char *scpi_backends = otc_buildinfo_scpi_backends_get();
	...
	version_info_.emplace_back("libsigrokdecode", QString("%1/%2 (rt: %3/%4)")
		.arg(otd_package_version_string_get(), otd_lib_version_string_get(),
	...
	// Protocol decoders
#ifdef ENABLE_DECODE
	GSList *sl = g_slist_copy((GSList *)otd_decoder_list());
	sl = g_slist_sort(sl, sort_pds);
	for (const GSList *l = sl; l; l = l->next) {
		dec = (struct otd_decoder *)l->data;
...
{
	cout << OTV_TITLE << " " << OTV_VERSION_STRING << endl;
