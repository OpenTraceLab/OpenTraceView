...
#ifndef PULSEVIEW_OTV_SUBWINDOWS_DECODERSELECTOR_SUBWINDOW_HPP
#define PULSEVIEW_OTV_SUBWINDOWS_DECODERSELECTOR_SUBWINDOW_HPP
...
#include "otv/subwindows/subwindowbase.hpp"
...
public:
	 ...
	 * ("logic", "uart", etc.)
	 */
	vector<const char*> get_decoder_inputs(const otd_decoder* d) const;

	/**
	 * Returns a list of protocol decoder IDs which provide a given output
	 * ("uart", "spi", etc.)
	 */
	vector<const otd_decoder*> get_decoders_providing(const char* output) const;

Q_SIGNALS:
	void new_decoders_selected(vector<const otd_decoder*> decoders);
...
#endif // PULSEVIEW_OTV_SUBWINDOWS_DECODERSELECTOR_SUBWINDOW_HPP
