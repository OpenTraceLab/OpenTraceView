...
const char *initial_notice =
	QT_TRANSLATE_NOOP("otv::subwindows::decoder_selector::SubWindow",
	...
	sort_filter_model_(new QCustomSortFilterProxyModel())
{
	...
	connect(this, SIGNAL(new_decoders_selected(vector<const otd_decoder*>)),
		&session, SLOT(on_new_decoders_selected(vector<const otd_decoder*>)));
...
vector<const char*> SubWindow::get_decoder_inputs(const otd_decoder* d) const
{
...
vector<const otd_decoder*> SubWindow::get_decoders_providing(const char* output) const
{
	vector<const otd_decoder*> ret_val;

	for (GSList* li = (GSList*)otd_decoder_list(); li; li = li->next) {
		const otd_decoder* d = (otd_decoder*)li->data;
		assert(d);
...
void SubWindow::on_item_changed(const QModelIndex& index)
{
	...
	if (index.isValid() && index.parent().isValid()) {
		...
		const otd_decoder* d = otd_decoder_get_by_id(decoder_name.toUtf8());

		id = QString::fromUtf8(d->id);
		longname = QString::fromUtf8(d->longname);
		desc = QString::fromUtf8(d->desc);
		doc = QString::fromUtf8(otd_decoder_doc_get(d)).trimmed();
...
void SubWindow::on_item_activated(const QModelIndex& index)
{
	...
	const otd_decoder* chosen_decoder = otd_decoder_get_by_id(decoder_name.toUtf8());
	if (chosen_decoder == nullptr)
		return;

	vector<const otd_decoder*> decoders;
	...
	while (strcmp(inputs.at(0), "logic") != 0) {
		vector<const otd_decoder*> prov_decoders = get_decoders_providing(inputs.at(0));
		...
		if (prov_decoders.size() == 1) {
			...
			QStringList items;
			for (const otd_decoder* d : prov_decoders)
			...
			decoders.push_back(otd_decoder_get_by_id(d.toUtf8()));
		}
