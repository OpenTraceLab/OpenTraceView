...
#ifndef PULSEVIEW_OTV_DATA_DECODESIGNAL_HPP
#define PULSEVIEW_OTV_DATA_DECODESIGNAL_HPP
...
#include <otv/data/decode/rowdata.hpp>
...
public:
	DecodeSignal(otv::Session &session);
	...
	void stack_decoder(const otd_decoder *decoder, bool restart_decode=true);
	...
	const otv::util::Timestamp start_time() const;
...
private:
	...
	Decoder* get_decoder_by_instance(const otd_decoder *const otd_dec);
	...
	void start_otd_session();
	void terminate_otd_session();
	void stop_otd_session();
	...
	static void annotation_callback(otd_proto_data *pdata, void *decode_signal);
	static void binary_callback(otd_proto_data *pdata, void *decode_signal);
	static void logic_output_callback(otd_proto_data *pdata, void *decode_signal);
...
private:
	...
	struct otd_session *otd_session_;
	...
	map<const otd_decoder*, shared_ptr<Logic>> output_logic_;
	map<const otd_decoder*, vector<uint8_t>> output_logic_muxed_data_;
...
#endif // PULSEVIEW_OTV_DATA_DECODESIGNAL_HPP
