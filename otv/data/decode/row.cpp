...
Row::Row(uint32_t index, Decoder* decoder, const otd_decoder_annotation_row* otd_row) :
	index_(index),
	decoder_(decoder),
	otd_row_(otd_row),
	visible_(true)
...
const otd_decoder_annotation_row* Row::get_otd_row() const
{
	return otd_row_;
}
...
{
	if (decoder_ && decoder_->name() && otd_row_ && otd_row_->desc)
		return QString("%1: %2")
			.arg(QString::fromUtf8(decoder_->name()),
			     QString::fromUtf8(otd_row_->desc));
	if (decoder_ && decoder_->name())
		return QString::fromUtf8(decoder_->name());
	if (otd_row_ && otd_row_->desc)
		return QString::fromUtf8(otd_row_->desc);
...
vector<AnnotationClass*> Row::ann_classes() const
{
	...
	for (GSList *l = otd_row_->ann_classes; l; l = l->next) {
...
bool Row::operator<(const Row& other) const
{
	return (decoder_ < other.decoder_) ||
		(decoder_ == other.decoder_ && otd_row_ < other.otd_row_);
}

bool Row::operator==(const Row& other) const
{
	return ((decoder_ == other.decoder()) && (otd_row_ == other.otd_row_));
}
