...
using otv::data::decode::AnnotationClass;
using otv::data::decode::DecodeChannel;
...
DecodeSignal::DecodeSignal(otv::Session &session) :
	SignalBase(nullptr, SignalBase::DecodeChannel),
	session_(session),
	otd_session_(nullptr),
...
void DecodeSignal::stack_decoder(const otd_decoder *decoder, bool restart_decode)
{
	assert(decoder);

	// Set name if this decoder is the first in the list or the name is unchanged
	const otd_decoder* prev_dec = stack_.empty() ? nullptr : stack_.back()->get_otd_decoder();
...
void DecodeSignal::reset_decode(bool shutting_down)
{
	...
	for (const shared_ptr<decode::Decoder>& dec : stack_)
		if (dec->has_logic_output())
			output_logic_[dec->get_otd_decoder()]->clear();
	...
	for (const shared_ptr<decode::Decoder>& dec : stack_) {
		...
		if (dec->has_logic_output()) {
			...
			if (!ch_exists) {
				...
				output_logic_[dec->get_otd_decoder()] = logic_data;
				output_logic_muxed_data_[dec->get_otd_decoder()] = vector<uint8_t>();
				...
				shared_ptr<Logic> logic_data = output_logic_[dec->get_otd_decoder()];
				logic_data->set_samplerate(get_samplerate());
...
const otv::util::Timestamp DecodeSignal::start_time() const
{
	otv::util::Timestamp result;
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
		settings.setValue("id", (const char *)decoder->get_otd_decoder()->id);
#else
		settings.setValue("id", decoder->get_otd_decoder()->id);
#endif
...
void DecodeSignal::restore_settings(QSettings &settings)
{
	...
	GSList *dec_list = g_slist_copy((GSList*)otd_decoder_list());
	...
	for (int decoder_idx = 0; decoder_idx < decoders; decoder_idx++) {
		...
		for (GSList *entry = dec_list; entry; entry = entry->next) {
			const otd_decoder *dec = (otd_decoder*)entry->data;
			if (!dec)
				continue;
...
Decoder* DecodeSignal::get_decoder_by_instance(const otd_decoder *const otd_dec)
{
	for (shared_ptr<Decoder>& d : stack_)
		if (d->get_otd_decoder() == otd_dec)
			return d.get();
...
void DecodeSignal::update_channel_list()
{
	...
	for (shared_ptr<Decoder>& decoder : stack_) {
		const otd_decoder* otd_dec = decoder->get_otd_decoder();
		const GSList *l;

		// Mandatory channels
		for (l = otd_dec->channels; l; l = l->next) {
			const struct otd_channel *const pdch = (struct otd_channel *)l->data;
			bool ch_added = false;
			...
			if (!ch_added) {
				...
				decode::DecodeChannel ch = {id++, 0, false, nullptr,
					...
					OTD_INITIAL_PIN_SAME_AS_SAMPLE0, decoder, pdch};
				channels_.push_back(ch);
			}
		}

		// Optional channels
		for (l = otd_dec->opt_channels; l; l = l->next) {
			const struct otd_channel *const pdch = (struct otd_channel *)l->data;
			bool ch_added = false;
	...
	const shared_ptr<const LogicSegment> input_segment)
{
	...
	for (int64_t i = abs_start_samplenum;
		...
		if (otd_session_send(otd_session_, i, chunk_end, chunk,
				data_size, unit_size) != OTD_OK) {
	...
	do {
		...
		if (!decode_interrupt_) {
			...
			if (input_segment->is_complete()) {
				...
				(void)otd_session_send_eof(otd_session_);
				new_annotations();
#endif
...
void DecodeSignal::start_otd_session()
{
	...
	if (otd_session_) {
		...
		if (samplerate)
			otd_session_metadata_set(otd_session_, OTD_CONF_SAMPLERATE,
				g_variant_new_uint64(samplerate));
		for (const shared_ptr<Decoder>& dec : stack_)
			dec->apply_all_options();
		otd_session_start(otd_session_);
	...
	// Create the session
	otd_session_new(&otd_session_);
	assert(otd_session_);

	// Create the decoders
	otd_decoder_inst *prev_di = nullptr;
	for (const shared_ptr<Decoder>& dec : stack_) {
		otd_decoder_inst *const di = dec->create_decoder_inst(otd_session_);

		if (!di) {
			set_error_message(tr("Failed to create decoder instance"));
			otd_session_destroy(otd_session_);
			otd_session_ = nullptr;
			return;
		}

		if (prev_di)
			otd_inst_stack(otd_session_, prev_di, di);
	...
	otd_pd_output_callback_add(otd_session_, OTD_OUTPUT_ANN,
		DecodeSignal::annotation_callback, this);

	otd_pd_output_callback_add(otd_session_, OTD_OUTPUT_BINARY,
		DecodeSignal::binary_callback, this);

	otd_pd_output_callback_add(otd_session_, OTD_OUTPUT_LOGIC,
...
void DecodeSignal::terminate_otd_session()
{
	...
	if (otd_session_) {
		...
		(void)otd_session_send_eof(otd_session_);
#endif
		otd_session_terminate_reset(otd_session_);
...
void DecodeSignal::stop_otd_session()
{
	if (otd_session_) {
		// Destroy the session
		otd_session_destroy(otd_session_);
...
void DecodeSignal::annotation_callback(otd_proto_data *pdata, void *decode_signal)
{
	assert(pdata);
	...
	assert(pdata->pdo);
	assert(pdata->pdo->di);
	const otd_decoder *const otd_dec = pdata->pdo->di->decoder;
	assert(otd_dec);

	const otd_proto_data_annotation *const pda = (const otd_proto_data_annotation*)pdata->data;
	assert(pda);

	// Find the row
	Decoder* dec = ds->get_decoder_by_instance(otd_dec);
	assert(dec);
...
void DecodeSignal::binary_callback(otd_proto_data *pdata, void *decode_signal)
{
	assert(pdata);
	assert(decode_signal);
	...
	const otd_proto_data_binary *const pdb = (const otd_proto_data_binary*)pdata->data;
	assert(pdb);
	...
	for (DecodeBinaryClass& bc : segment->binary_classes)
		if ((bc.decoder->get_otd_decoder() == otd_dec) &&
...
void DecodeSignal::logic_output_callback(otd_proto_data *pdata, void *decode_signal)
{
	assert(pdata);
	assert(decode_signal);
	...
	assert(pdata->pdo);
	assert(pdata->pdo->di);
	const otd_decoder *const decc = pdata->pdo->di->decoder;
	assert(decc);

	const otd_proto_data_logic *const pdl = (const otd_proto_data_logic*)pdata->data;
	assert(pdl);
