...
#include <otv/binding/decoder.hpp>
...
SignalBase::SignalBase(shared_ptr<opentrace::Channel> channel, ChannelType channel_type) :
	channel_(channel),
...
shared_ptr<opentrace::Channel> SignalBase::channel() const
{
	return channel_;
}
...
void SignalBase::set_data(shared_ptr<otv::data::SignalData> data)
{
	if (data_) {
...
shared_ptr<otv::data::SignalData> SignalBase::data() const
{
	return data_;
}
	...
	shared_ptr<LogicSegment> lsegment, uint64_t start_sample, uint64_t end_sample)
{
	if (end_sample > start_sample) {
		...
		// Create opentrace::Analog instance
		float *asamples = new float[ConversionBlockSize];
		assert(asamples);
		uint8_t *lsamples = new uint8_t[ConversionBlockSize];
		assert(lsamples);

		vector<shared_ptr<opentrace::Channel> > channels;
		if (channel_)
			channels.push_back(channel_);

		vector<const opentrace::QuantityFlag*> mq_flags;
		const opentrace::Quantity * const mq = opentrace::Quantity::VOLTAGE;
		const opentrace::Unit * const unit = opentrace::Unit::VOLT;

		shared_ptr<opentrace::Packet> packet =
			Session::otc_context->create_analog_packet(channels,
			asamples, ConversionBlockSize, mq, unit, mq_flags);

		shared_ptr<opentrace::Analog> analog =
			dynamic_pointer_cast<opentrace::Analog>(packet->payload());
		...
		if (conversion_type_ == A2LConversionByThreshold) {
			...
			while ((end_sample - i) > ConversionBlockSize) {
				...
				shared_ptr<opentrace::Logic> logic =
			...
			// Re-create opentrace::Analog and convert remaining samples
			packet = Session::otc_context->create_analog_packet(channels,
				asamples, end_sample - i, mq, unit, mq_flags);

			analog = dynamic_pointer_cast<opentrace::Analog>(packet->payload());
