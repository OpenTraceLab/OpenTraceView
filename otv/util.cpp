...
// Timestamp is now just a double, so we use the built-in QTextStream operator
...
QString format_time_minutes(const Timestamp& t, signed precision, bool sign)
{
	...
	const unsigned int hours = (uint)fmod(whole_seconds / (60 * 60), 24);
	const unsigned int minutes = (uint)fmod(whole_seconds / 60, 60);
	const unsigned int seconds = (uint)fmod(whole_seconds, 60);
