...
using namespace otv::views::trace;

namespace {
	QString format(const otv::util::Timestamp& t)
	{
		return otv::util::format_time_si(t, otv::util::SIPrefix::none, 6);
	}
...
{
	const otv::util::Timestamp major_period("0.1");
	const otv::util::Timestamp offset("0");
...
{
	const otv::util::Timestamp major_period("0.1");
	const otv::util::Timestamp offset("-0.463");
	const double scale(0.001);
	const int width(500);
	const unsigned int minor_tick_count(4);
...
{
	const otv::util::Timestamp major_period("20");
	const otv::util::Timestamp offset("8");
