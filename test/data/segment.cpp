...
BOOST_AUTO_TEST_CASE(SmallSize8Single)
{
	...
	//----- Chunk size << otv::data::Segment::MaxChunkSize @ 8bit, added in 1 call ----//
...
{
	Segment s(0, 1, sizeof(uint8_t));
	uint32_t num_samples = otv::data::Segment::MaxChunkSize;
...
BOOST_AUTO_TEST_CASE(MaxSize8Single)
{
	...
	uint32_t num_samples = 2*otv::data::Segment::MaxChunkSize;
...
BOOST_AUTO_TEST_CASE(MediumSize24Single)
{
	...
	// Chunk size is num*unit_size, so with otv::data::Segment::MaxChunkSize/unit_size, we reach the maximum size
	uint32_t num_samples = otv::data::Segment::MaxChunkSize / 3;

	//----- Chunk size == otv::data::Segment::MaxChunkSize @ 24bit, added in 1 call ----//
...
BOOST_AUTO_TEST_CASE(MediumSize32Single)
{
	...
	// Chunk size is num*unit_size, so with otv::data::Segment::MaxChunkSize/unit_size, we reach the maximum size
	uint32_t num_samples = otv::data::Segment::MaxChunkSize / sizeof(uint32_t);

	//----- Chunk size == otv::data::Segment::MaxChunkSize @ 32bit, added in 1 call ----//
...
BOOST_AUTO_TEST_CASE(MaxSize32Single)
{
	...
	uint32_t num_samples = 2*(otv::data::Segment::MaxChunkSize / sizeof(uint32_t));
...
BOOST_AUTO_TEST_CASE(MediumSize32Multi)
{
	...
	//----- Chunk size == otv::data::Segment::MaxChunkSize @ 32bit, added in num_samples calls ----//
	uint32_t data;
	for (uint32_t i = 0; i < num_samples; i++) {
		data = i;
...
BOOST_AUTO_TEST_CASE(MaxSize32MultiAtOnce)
{
	...
	// Chunk size is num*unit_size, so with otv::data::Segment::MaxChunkSize/unit_size, we reach the maximum size
	uint32_t num_samples = 3*(otv::data::Segment::MaxChunkSize / sizeof(uint32_t));
...
BOOST_AUTO_TEST_CASE(MaxSize32MultiIterated)
{
	...
	otv::data::SegmentDataIterator* it = s.begin_sample_iteration(0);
